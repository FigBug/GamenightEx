...
$sql = "select games.date, games.gameid, scores.name, elo1.elo from elo1 inner join games on games.gameid = elo1.gameid inner join scores on games.gameid = scores.gameid where elo1.name in (select scores.name from scores inner join games on games.gameid=scores.gameid where games.name='$game' and games.number = 1 group by scores.name having count(*) >= 2) and scores.name = elo1.name and games.name='$game' and games.number = 1 order by games.date, games.number asc";
